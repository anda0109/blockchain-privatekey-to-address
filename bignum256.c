...
//\return <0: error
int byteFromHexString(char h1, char h2)
{
//	printf("%c%c", h1, h2);
...
//\param str The hex string without 0x
//\param out The output
//\return 0 if ok 1, !0 if failed

int bigFromHexString(const char *str, BigNum256 out)
{
	int i;
	int ret;
	if(str == NULL) return 1;
	if(strlen(str) != BIGNUM256_HEX_STRING_LEN) return 2;

	for(i = 0; i < 32; i++)
	{
		ret = byteFromHexString(str[(31-i)*2], str[(31-i)*2+1]);
		if(ret < 0)	return -1;

		out[i] = (unsigned char)ret;
	}
	return 0;
}
void bigToHexString(const BigNum256 in, char str[BIGNUM256_HEX_STRING_LEN + 1])
{
	int i;
	str[BIGNUM256_HEX_STRING_LEN] = 0;

	for(i = 0; i < 32; i++)
	{
		if((in[31-i] >> 4) >= 10) 
			str[i*2] = (char)((in[31-i] >> 4) - 10 + 'A');
		else
			str[i*2] = (char)((in[31-i] >> 4) + '0');

		if((in[31-i] & 0x0F) >= 10) 
			str[i*2 + 1] = (char)((in[31-i] & 0x0F) - 10 + 'A');
		else
			str[i*2 + 1] = (char)((in[31-i] & 0x0F) + '0');
	}
}
//BTC
void bigToHexStringDirectWithout0x(const BigNum256 in, char str[BIGNUM256_HEX_STRING_LEN + 3])
{
	int i;
	...
	{
		if((in[i] >> 4) >= 10) 
			str[i*2] = (char)((in[i] >> 4) - 10 + 'A');
		else
			str[i*2] = (char)((in[i] >> 4) + '0');

		if((in[i] & 0x0F) >= 10) 
			str[i*2 + 1] = (char)((in[i] & 0x0F) - 10 + 'A');
		else
			str[i*2 + 1] = (char)((in[i] & 0x0F) + '0');
	}
}
